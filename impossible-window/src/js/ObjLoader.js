// IMPORTANT: You must check Triangulate Faces and Objects as Objects
// in blender exporter
// obj model loader
// fork from FunWithWebGL2\lesson_013
class ObjLoader{
  // static domToMesh(gl, elmID, meshName, flipYUV, keepRawData){
    ...
    while(posB > posA) {
      ...
      switch(line.charAt(0)) {
        case 'o':
          ...
          if(fVert.length > 0) {
            pushToModels(fVert, fNorm, fUV, fIndex, name);
            aCache = [];
            fIndexCnt = 0;
            fVert = []; fNorm = []; fUV = []; fIndex = [];
          }
          name = itm[0];
        break;
