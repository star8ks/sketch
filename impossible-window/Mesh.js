...
function Mesh(regl, {positions, cells, normals, matrix, name=''}, center=[0, 0, 0]) {
  this.center = center;
  this.name = name;
  ...
  this.draw = regl({
    ...
    uniforms: {
      ...
      'lights[0].color': [0.5, 0.003, 0.152],
      'lights[1].color': normalRGB(216, 7, 59),//[0.85, 0.027, 0.26],//
      'lights[2].color': normalRGB(255, 12, 84),//[0.788, 0.011, 0.243],
      // 'lights[0].color': [1, 0, 0],
      // 'lights[1].color': [0, 1, 0],
      // 'lights[2].color': [0, 0, 1],
      'lights[0].position': [10, 0, 0],
      // 'lights[0].position': ({tick}) => [10 * (1+Math.sin(tick*0.01)), 10 * Math.cos(tick*0.01), 0],
      'lights[1].position': ({ tick }, props) => {
        if (props.gameEnd) {
      ...
      'lights[2].position': ({ tick }, props) => {
        if (props.gameEnd) {
          // todo: move to animation
          return [0, 10, 10];//[0, 0, 10];
        } else {
          return [0, 0, 1];
        }
      },
